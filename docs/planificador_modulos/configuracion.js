    ...
    function createConfiguracion(ctx){
        ...
        function resumenMemoriaPlanificacion(){
            ...
            const labelTipo={
                ...
                auto_general_omitida:'Omisiones Auto-general',
                auto_general_estrategia_aceptada:'Estrategias aceptadas'
            };
                    ...
                    <div class="help-layout">
                        ...
                        <div class="help-content">
                            ...
                            <section id="ayuda-docentes">
                                ...
                                <p><strong>Ejemplo:</strong> si un docente de referencia puede dictar EEA401 y EEA402, esas asignaturas deben estar asociadas para que el sistema lo proponga correctamente.</p>
                            </section>
                    ...
                    <div class="help-layout">
                        ...
                        <div class="help-content">
                            ...
                            <section id="dev-iteraciones">
                                ...
                                <p>El desarrollo avanzó mediante un proceso de corrección guiada. La coordinación académica probaba la app en escenarios reales, detectaba una incoherencia y la convertía en una regla funcional. Cada ciclo permitió transformar conocimiento tácito de planificación en comportamiento del sistema.</p>
                                <ul>
...
La interfaz debe ser profesional, sobria, modular, con desplazamiento horizontal/vertical cuando sea necesario, sin romper la estructura al reducir ventana. Debe incluir ayuda de usuario y documentación de desarrollo dentro del menú Configuración.</pre>
        ...
        function abrirConfiguracion(){
            ...
            const solverLabels={
                topesDuros:'Evitar topes docente/sala/sección',
                bloquesFaltantes:'Completar horas requeridas',
                disponibilidadDocente:'Respetar disponibilidad docente',
                topesSala:'Evitar topes de sala',
                topesSeccion:'Evitar topes de sección',
                ventanasEstudiantes:'Evitar ventanas de estudiantes',
                ventanasDocentes:'Evitar ventanas docentes',
                compactacionAsignatura:'Compactar bloques de asignatura',
                compactacionLarga:'Compactar asignaturas de 54/72 horas',
                distribucionSemanal:'Distribuir carga semanal',
                distribuirAsignaturas:'Separar asignaturas distintas por día',
                salasCorrectas:'Preferir salas definitivas/sugeridas',
                docentePreferente:'Preferir docente apto/preferente',
                excesoDiarioDocente:'Controlar máximo diario docente',
                respetoJornada:'Respetar jornada de la sección',
                descansoDocente:'Respetar descanso docente',
                virtuales:'Ubicar bloques virtuales con criterio',
                virtualesSabado:'Priorizar virtuales sábado',
                onlineTeams:'Respetar Online TEAMS',
                sabadoPresencial:'Evitar sábado presencial',
                evitarTemprano:'Evitar bloques tempranos',
                primerSemestre:'Cuidar primer semestre/N1',
                criticas:'Cuidar asignaturas críticas',
                ayudantias:'Reservar margen para ayudantías',
                recreo:'Tratar recreo como recomendación',
                docenteNN:'Penalizar Docente NN',
                tro2:'Penalizar TRO2/Terreno',
                transversalesHeredadas:'Cuidar herencias y transversales',
                continuidadJornada:'Preferir continuidad por jornada',
                vespertinoBase:'Construir vespertino desde la base',
                memoriaManual:'Considerar memoria de correcciones'
            };
            const solverDetalles={
                topesDuros:'Agrupa los topes principales. Mantener obligatorio refuerza que no se generen choques básicos.',
                bloquesFaltantes:'Prioriza completar las horas presenciales y virtuales que exige cada asignatura.',
                disponibilidadDocente:'Evita ubicar clases donde el docente no declaró disponibilidad.',
                topesSala:'Evita que dos asignaturas usen la misma sala en el mismo bloque.',
                topesSeccion:'Evita que una sección tenga dos clases en el mismo bloque.',
                ventanasEstudiantes:'Reduce espacios vacíos dentro del día de una sección.',
                ventanasDocentes:'Reduce espacios vacíos dentro del día de un docente.',
                compactacionAsignatura:'Favorece bloques consecutivos de la misma asignatura.',
                compactacionLarga:'Refuerza compactación en asignaturas de 54/72 horas, donde conviene trabajar corrido.',
                distribucionSemanal:'Evita concentrar demasiada carga en pocos días.',
                distribuirAsignaturas:'Intenta que asignaturas distintas no caigan todas el mismo día.',
                salasCorrectas:'Prefiere sala sugerida, sala definitiva o tipo de espacio adecuado.',
                docentePreferente:'Premia docentes marcados como preferente/apto para la asignatura.',
                excesoDiarioDocente:'Controla que presencial + virtual no supere el máximo diario.',
                respetoJornada:'Evita sacar diurno/vespertino de sus bloques naturales.',
                descansoDocente:'Cuida el descanso mínimo entre jornadas de un docente.',
                virtuales:'Ubica virtuales sin generar ventanas innecesarias.',
                virtualesSabado:'Prioriza sábado para virtuales cuando corresponde al nivel/sección.',
                onlineTeams:'Protege reglas propias de asignaturas Online TEAMS.',
                sabadoPresencial:'Penaliza clases presenciales en sábado salvo autorización.',
                evitarTemprano:'Reduce uso de B1/B2 cuando no es necesario.',
                primerSemestre:'Refuerza el cuidado especial de estudiantes nuevos.',
                criticas:'Prioriza horarios más razonables para asignaturas críticas.',
                ayudantias:'Deja margen para planificar ayudantías cuando corresponde.',
                recreo:'Evita cortes incómodos alrededor de recreo, sin bloquearlos totalmente.',
                docenteNN:'Penaliza dejar clases con Docente NN cuando existe alternativa.',
                tro2:'Penaliza dejar salas TRO2/Terreno cuando existe sala definida.',
                transversalesHeredadas:'Cuida asignaturas heredadas, fusionadas y de coordinación externa.',
                continuidadJornada:'Favorece continuidad lógica dentro de la jornada.',
                vespertinoBase:'Prioriza compactar vespertinos desde los bloques base.',
                memoriaManual:'Permite que correcciones manuales previas influyan en nuevas propuestas.'
            };
            const solverGrupos=[
                {titulo:'Restricciones duras',detalle:'Criterios que protegen factibilidad básica.',keys:['topesDuros','disponibilidadDocente','topesSala','topesSeccion','respetoJornada','descansoDocente','bloquesFaltantes']},
                {titulo:'Calidad del horario',detalle:'Criterios que hacen más cómodo el horario para estudiantes y docentes.',keys:['ventanasEstudiantes','ventanasDocentes','compactacionAsignatura','compactacionLarga','distribucionSemanal','distribuirAsignaturas','continuidadJornada','recreo']},
                {titulo:'Recursos académicos',detalle:'Criterios asociados a docentes, salas y placeholders.',keys:['docentePreferente','salasCorrectas','excesoDiarioDocente','docenteNN','tro2']},
                {titulo:'Modalidades y casos especiales',detalle:'Criterios propios de virtuales, Online, transversales y criterios académicos.',keys:['virtuales','virtualesSabado','onlineTeams','sabadoPresencial','evitarTemprano','primerSemestre','criticas','ayudantias','transversalesHeredadas','vespertinoBase','memoriaManual']}
            ];
            ...
            const selectorPesoSolver=(key,pesos)=>`
                <div class="motor-criterion">
                    <div>
                        <label class="form-label" for="cfgSolver_${key}">${solverLabels[key]||key}</label>
                        <small>${solverDetalles[key]||''}</small>
                    </div>
                    <select class="form-select cfg-solver-peso" id="cfgSolver_${key}" data-solver-key="${key}">
                        ${solverOpciones.map(([v,l])=>`<option value="${v}" ${pesos[key]===v?'selected':''}>${l}</option>`).join('')}
                    </select>
                </div>`;
            ...
            const configDefault=JSON.parse(JSON.stringify(ctx.CONFIG_DEFAULT||{}));
            const aplicarConfiguracionInicialABorrador=()=>{
                const preservados={
                    temporadaActualId:cfg.temporadaActualId
                };
                Object.keys(cfg).forEach(k=>delete cfg[k]);
                Object.assign(cfg,JSON.parse(JSON.stringify(configDefault)),preservados);
                cfg.solverPesos=Object.assign({},solverDefault,configDefault.solverPesos||{});
                cfg.dashboard=Object.assign({},configDefault.dashboard||{});
            };
            const cambiosCriticosConfiguracion=(prev,next)=>{
                const cambios=[];
                if(Number(prev.bloquesDiariosMax)!==Number(next.bloquesDiariosMax)) cambios.push(`Bloques diarios máximos: ${prev.bloquesDiariosMax} → ${next.bloquesDiariosMax}`);
                if(Number(prev.bloquesSemestralesMax)!==Number(next.bloquesSemestralesMax)) cambios.push(`Bloques semestrales máximos: ${prev.bloquesSemestralesMax} → ${next.bloquesSemestralesMax}`);
                if(Number(prev.horasDescanso)!==Number(next.horasDescanso)) cambios.push(`Horas mínimas de descanso: ${prev.horasDescanso} → ${next.horasDescanso}`);
                if(Number(prev.sabadoHastaBloque)!==Number(next.sabadoHastaBloque)) cambios.push(`Sábado hasta bloque: B${prev.sabadoHastaBloque} → B${next.sabadoHastaBloque}`);
                const clavesDuras=['topesDuros','bloquesFaltantes','disponibilidadDocente','topesSala','topesSeccion','respetoJornada','descansoDocente','onlineTeams'];
                clavesDuras.forEach(k=>{
                    const antes=normalizarNivelSolver(prev.solverPesos?.[k],solverDefault[k]);
                    const despues=normalizarNivelSolver(next.solverPesos?.[k],solverDefault[k]);
                    if(antes!==despues) cambios.push(`${solverLabels[k]||k}: ${antes} → ${despues}`);
                });
                return cambios;
            };
            ...
            <div class="modal-overlay" id="modalOverlay"><div class="modal">
                ...
                <div class="config-section">
                    <h4>Motor de planificación</h4>
                    <div class="config-help-entry">
                        <div>
                            <p>Configura las prioridades que influyen en auto-planificación y optimización sin sobrecargar este menú.</p>
                            <small style="color:var(--text-secondary);font-size:0.72rem;">${Object.keys(solverDefault).length} criterios disponibles · cambios con Guardar/Cancelar propio.</small>
                        </div>
                        <div class="config-help-actions">
                            <button class="btn btn-primary btn-sm" id="btnAbrirMotorPlanificacion" type="button">Configurar motor</button>
                        </div>
                    </div>
                </div>
                <div class="config-section">
                ...
                <div class="config-section">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group"><label class="form-label">Nombre visible</label><input class="form-input" id="cfgPerfilNombre" value="${ctx.escapeAttr(perfilActual.nombre||'')}" placeholder="Ej. Coordinador Académico"></div>
                    </div>
                ...
                <div class="modal-actions split-actions">
                    <button class="btn" id="btnRestaurarConfig" type="button">Restaurar</button>
                ...
                const renderMotor=()=>{
                    const html=solverGrupos.map(grupo=>`
                        <section class="motor-group">
                            <div class="motor-group-header">
                                <h4>${ctx.escapeHTML(grupo.titulo)}</h4>
                                <p>${ctx.escapeHTML(grupo.detalle)}</p>
                            </div>
                            <div class="motor-criteria-grid">
                                ${grupo.keys.map(key=>selectorPesoSolver(key,draft)).join('')}
                            </div>
                        </section>`).join('');
                    const existente=document.getElementById('motorOverlay');
                    if(existente) existente.remove();
                    document.body.insertAdjacentHTML('beforeend',`
                        <div class="modal-overlay motor-overlay" id="motorOverlay">
                            <div class="modal motor-modal">
                                <div class="modal-header">
                                ...
                                <div class="motor-scale">
                                    ...
                                    <span><b>Alta</b> prioridad fuerte</span>
                                    <span><b>Media</b> equilibrio</span>
                                    <span><b>Baja</b> sugerencia leve</span>
                                    <span><b>Ignorar</b> no participa</span>
                                </div>
                                <div class="motor-groups">${html}</div>
                                <div class="modal-actions motor-actions split-actions">
                                    <button class="btn" id="btnRestaurarMotor" type="button">Restaurar</button>
                                    <div class="action-right">
                                        <button class="btn" id="btnCancelarMotor" type="button">Cancelar</button>
                                        <button class="btn btn-primary" id="btnGuardarMotor" type="button">Guardar</button>
                                    </div>
                                </div>
                            </div>
                        </div>`);
                    ...
                    document.getElementById('btnRestaurarMotor')?.addEventListener('click',async()=>{
                        const ok=ctx.confirmarAccionCritica
                            ? await ctx.confirmarAccionCritica({
                                titulo:'Restaurar motor de planificación',
                                mensaje:'El borrador del motor volverá al seteo inicial.',
                                detalles:['No se aplicará definitivamente hasta presionar Guardar.', 'Puede cambiar la forma en que auto-planificación y optimizador priorizan los bloques.'],
                                confirmarTexto:'Restaurar motor',
                                peligro:false
                            })
                    ...
                    document.getElementById('btnGuardarMotor')?.addEventListener('click',()=>{
                        ...
                        cerrar();
                        ctx.toast('Motor de planificación actualizado','success');
                    });
                };
                renderMotor();
            }
            document.getElementById('btnAbrirMotorPlanificacion')?.addEventListener('click',abrirMotorPlanificacion);
            document.getElementById('btnRestaurarConfig')?.addEventListener('click',async()=>{
                const ok=ctx.confirmarAccionCritica
                    ? await ctx.confirmarAccionCritica({
                        titulo:'Restaurar configuración',
                        mensaje:'La configuración volverá al seteo inicial de la app.',
                        detalles:['Se conservará solo la temporada activa para no dejar la app sin contexto.', 'No se aplicará definitivamente hasta presionar Guardar cambios.'],
                        confirmarTexto:'Restaurar configuración',
                        peligro:false
                    })
                    : confirm('¿Restaurar la configuración inicial? Podrás confirmar con Guardar.');
                if(!ok) return;
                aplicarConfiguracionInicialABorrador();
                ctx.cerrarModal();
                abrirConfiguracion();
                ctx.toast('Configuración restaurada en borrador. Presiona Guardar cambios para aplicar.','info');
            });
            ...
            document.getElementById('btnGuardarConfig').addEventListener('click',async()=>{
                const configAnterior=JSON.parse(JSON.stringify(data.configuracion||{}));
                ...
                if(criticos.length){
                    const ok=ctx.confirmarAccionCritica
                        ? await ctx.confirmarAccionCritica({
                            titulo:'Confirmar reglas críticas',
                            mensaje:'Estás modificando reglas que influyen directamente en factibilidad, auto-planificación u optimización.',
                            detalles:criticos.slice(0,8).concat(criticos.length>8?`${criticos.length-8} cambio(s) adicional(es).`:[]),
                            confirmarTexto:'Guardar reglas',
                            peligro:false
                        })
                        : confirm('Estás modificando reglas críticas. ¿Guardar cambios?');
                    if(!ok) return;
                }
            ...
            document.getElementById('btnAgregarTemp').onclick=()=>{
                ...
                if (copiar) {
                    ...
                    data.temporadaData[id] = {
                        carreras:JSON.parse(JSON.stringify(src.carreras||[])),
                        niveles:JSON.parse(JSON.stringify(src.niveles||[])),
                        secciones:JSON.parse(JSON.stringify(src.secciones||[])),
                        asignaturas:JSON.parse(JSON.stringify(src.asignaturas||[])),
                        docentes:JSON.parse(JSON.stringify(src.docentes||[])),
                        salas:JSON.parse(JSON.stringify(src.salas||[])),
                        asignaturaCarreraNivel:JSON.parse(JSON.stringify(src.asignaturaCarreraNivel||[])),
