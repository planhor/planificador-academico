...
(() => {
    ...
    function normalizarGestorSeccionesData(valor){
        const base={cargas:[],ids:[],filas:[],enlacesManuales:[],ultimaCargaId:null,tablaLimite:250};
        ...
        return {cargas,ids,filas,enlacesManuales,ultimaCargaId:limpiarTexto(valor.ultimaCargaId)||cargas[0]?.id||null,tablaLimite:Number(valor.tablaLimite)||250};
    }
    ...
    function analizarGestorSecciones(aoa){
        ...
        filas.slice(0,12).forEach((row,idx)=>{
    ...
    function normalizarPropuestaGestor(registros){
        ...
        const grupos=Array.from(gruposCandidatos.values()).map(g=>{
            if(!g.planificada && g.fusionadas.length) g.planificada=g.fusionadas[0];
        ...
        return {
            ...
            seccionAsignaturas:Array.from(seccionAsignaturas.values()),
            grupos
        };
    }
    ...
    function construirMemoriaIdsGestor(analisis,nombreArchivo){
        const cargaId=genId();
        ...
        const ids=Array.from(grupos.values()).map(g=>{
            ...
            }else if(g.seccionesReales.length){
                g.estado='resuelta';
                g.madreDetectada=g.seccionesReales[0];
                g.motivo=g.filasFusionadas?`${g.filasFusionadas} fila(s) fusionadas trazadas por ID.`:'ID con sección real visible.';
            }else{
    ...
    function guardarMemoriaGestor(analisis,nombreArchivo){
        const memoria=construirMemoriaIdsGestor(analisis,nombreArchivo);
        data.gestorSecciones=normalizarGestorSeccionesData(data.gestorSecciones);
    ...
    function renderGestorSecciones(){
        ...
        idsEl.innerHTML=muestra.length?`
            <div class="gestor-mini-table"><table>
                <thead><tr><th>ID</th><th>Estado</th><th>Asignatura</th><th>Madre detectada</th><th>Secciones reales</th><th>Motivo</th></tr></thead>
    ...
    function mostrarPreviewGestorSecciones(analisis,nombreArchivo){
        ...
        const muestrasGrupos=propuesta.grupos.slice(0,8).map(g=>`
            <tr>
                ...
                <td>${escapeHTML(g.planificada?.seccion||'')}</td>
                <td>${escapeHTML(g.fusionadas.map(f=>f.seccion).join(', '))}</td>
        ...
        document.getElementById('modalContainer').innerHTML=`
            <div class="modal-overlay" id="modalOverlay"><div class="modal auto-general-modal">
                ...
                <div class="dashboard-detail-table" style="margin-bottom:12px;">
                    ...
                    <table class="report-table">
                        <thead><tr><th>Asignatura</th><th>Sección madre tentativa</th><th>Secciones heredadas tentativas</th><th>N°</th><th>Alumnos</th></tr></thead>
                        <tbody>${muestrasGrupos||'<tr><td colspan="5">Sin grupos candidatos todavía</td></tr>'}</tbody>
                    </table>
                </div>
                ...
                <div class="modal-actions">
                    ...
                    <button class="btn btn-primary" id="btnAplicarPreviewGestor" ${revision.conteo.criticos||analisis.faltantes.length?'disabled':''}>Aplicar propuesta</button>
                </div>
            </div></div>`;
        const cerrar=()=>{ document.getElementById('modalContainer').innerHTML=''; };
        ...
        document.getElementById('btnAplicarPreviewGestor')?.addEventListener('click',()=>{
            if(revision.conteo.criticos||analisis.faltantes.length){
        ...
        input.onchange=e=>{
            ...
            reader.onload=ev=>{
                try{
                    ...
                    guardarMemoriaGestor(analisis,file.name);
                    normalizarDatos();
                    guardar({forzar:true});
                    renderGestorSecciones();
