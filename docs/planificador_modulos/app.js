...
(() => {
    ...
    let modalDragInicioDentro = false;
    document.addEventListener('pointerdown',(e)=>{
        const overlay=e.target.closest?.('.modal-overlay');
        if(!overlay) {
            modalDragInicioDentro=false;
            return;
        }
        modalDragInicioDentro=!!e.target.closest?.('.modal');
    },true);
    document.addEventListener('click',(e)=>{
        ...
        if(e.target===overlay && modalDragInicioDentro){
            e.preventDefault();
    ...
    async function limpiarPlanificacion(){
        ...
        const ok=await confirmarAccionCritica({
            ...
            detalles:[
                `${total} bloque(s) planificado(s) se eliminarán.`,
                'No se eliminarán secciones, asignaturas, docentes ni salas.',
                'Los reportes se recalcularán vacíos o con pendientes según los datos restantes.'
            ],
    ...
    async function limpiarTemporada(){
        ...
        const ok=await confirmarAccionCritica({
            ...
            detalles:[
                'Se eliminarán carreras, niveles, secciones, asignaturas, docentes, salas ingresadas, Gestor y planificación de esta temporada.',
                'No se eliminarán otras temporadas.',
                'No se modificarán las configuraciones globales.'
            ],
    ...
    async function limpiarTodoApp(){
        const ok=await confirmarAccionCritica({
            ...
            detalles:[
                `${data.temporadas?.length||0} temporada(s) se reemplazarán por una temporada nueva vacía.`,
                'No se restaurarán configuraciones ni preferencias del motor.',
                'Esta acción no elimina usuarios ni datos de Firebase Authentication.'
            ],
