            ...
            ctx.BLOQUES.forEach(b=>{
                ...
                ctx.DIAS.forEach((d,di)=>{
                    ...
                    if(plan){
                        ...
                        html+='<td style="padding:2px 1px;border:1px solid var(--border);text-align:center;vertical-align:middle;word-wrap:break-word;background:'+ctx.colorSeguro(asig?.color,'#e9ecef')+';font-size:0.6rem;line-height:1.3;">'+ctx.escapeHTML(asig?.codigo||'?')+'<br><span style="font-size:0.55rem;">'+ctx.escapeHTML(sala?sala.nombre:'')+'</span><br><span style="font-size:0.55rem;">'+ctx.escapeHTML(sec?sec.nombre:'')+'</span></td>';
                    } else if(!disponible){
                        html+='<td style="padding:2px;border:1px solid var(--border);text-align:center;background:#e0e0e0;color:#bbb;font-size:0.6rem;height:48px;">x</td>';
                    } else {
