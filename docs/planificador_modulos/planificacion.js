(function(){
    function createPlanificacion(ctx){
        ...
        function aplicarEstadoCelda(cell,plan,meta={}){
            ...
            if(meta.vinculado) cell.title=`Bloque vinculado desde ${nombreSeccion(plan.seccionId)}. Se edita desde la sección madre.${compNombre?` Componente: ${compNombre}.`:''}`;
            else if(explicacion) cell.title=explicacion;
            else if(compNombre) cell.title=`Componente: ${compNombre}`;
        }
        ...
        function posicionarPopupEnPuntero(popup, cell, evento=null){
            ...
            const x=Number.isFinite(evento?.clientX)?evento.clientX:rect.left;
            const y=Number.isFinite(evento?.clientY)?evento.clientY:rect.bottom;
            const margen=8;
            const ancho=popup.offsetWidth||240;
            const alto=popup.offsetHeight||220;
            let left=x+margen;
            let top=y+margen;
            if(left+ancho>window.innerWidth-margen) left=Math.max(margen,x-ancho-margen);
            if(top+alto>window.innerHeight-margen) top=Math.max(margen,y-alto-margen);
        ...
        function actualizarProgresoPlan(){
            ...
            el.innerHTML=`${etiquetaPresencial}: <span style="color:${colorP};font-weight:600;">${planesComp.length}/${reqComp}</span> · Virtual: <span style="color:${colorV};font-weight:600;">${planesV.length}/${reqV}</span> · Total operativo: ${totalPlan}/${totalReq}${nota}${capacidad}`;
            el.style.display='block';
            actualizarMensajesPlanificador();
        }
        ...
        function init(){
            ...
            const celdaDesdeEvento=(e)=>{
                ...
                return bajoPuntero?.closest?.('.grid-cell')||null;
            };
            ...
            scheduleGrid.addEventListener('pointerdown',(e)=>{
                ...
                const cell=celdaDesdeEvento(e); if(!cell||cell.dataset.dia===undefined) return; if(e.button!==0) return;
