        ...
        function init(){
            ...
            const celdaDesdeEvento=(e,opciones={})=>{
                ...
                if(preferirPuntero && Number.isFinite(e?.clientX) && Number.isFinite(e?.clientY)){
                    ...
                    const celda=bajoPuntero?.closest?.('.grid-cell')||null;
            ...
            const finalizarArrastre=(e)=>{
                ...
                if(!arrastre.activo) return;
                const celdaFinal=celdaDesdeEvento(e);
                if(celdaFinal?.dataset?.dia!==undefined && !arrastre.celdas.some(c=>c.dia===Number(celdaFinal.dataset.dia)&&c.bloque===Number(celdaFinal.dataset.bloque))){
                    arrastre.celdas.push({dia:Number(celdaFinal.dataset.dia),bloque:Number(celdaFinal.dataset.bloque),cell:celdaFinal});
                }
                ...
                if(data.modoPlan){
                    ...
                    if(estado){
                        ...
                        celdas.forEach(c=>{
                            ...
                            if(plan){ eliminarBloque(plan); cont++; }
                        });
