(function(){
    function createPlanificacion(ctx){
        ...
        let arrastre={activo:false,inicioDia:null,inicioBloque:null,estado:false,celdas:[],origenPlan:null,ghost:null,grupoMovimiento:null};
        ...
        function alumnosGrupoAsignatura(asigId,seccionId){
            const estado=estadoDictacionAsignatura(asigId,seccionId);
        ...
        function evaluarCapacidadSala(asigId,seccionId,salaId){
            const alumnos=alumnosGrupoAsignatura(asigId,seccionId);
        ...
        function confirmarCapacidadSala(asigId,seccionId,salaId){
            const evalCap=evaluarCapacidadSala(asigId,seccionId,salaId);
        ...
        function notaCapacidadPlan(plan){
            ...
            const evalCap=evaluarCapacidadSala(plan.asignaturaId,plan.seccionId,plan.salaId);
        ...
        function asignarBloque(dia,bloque){
            ...
            if(s.tipo==='presencial' && !confirmarCapacidadSala(s.asignaturaId,s.seccionId,salaPlan)) return false;
            ctx.pushUndo();
            const plan={id:ctx.genId(),seccionId:s.seccionId,asignaturaId:s.asignaturaId,docenteId:s.docenteId,salaId:salaPlan,dia,bloque,tipoPresencial:s.tipo==='presencial'};
        ...
        function cambiarSalaAsignatura(plan){
            ...
            document.getElementById('btnAplicarCambioSala')?.addEventListener('click',()=>{
                ...
                for(const salaId of salasPorConfirmar){
                    if(!confirmarCapacidadSala(plan.asignaturaId,plan.seccionId,salaId)) return;
                }
        ...
        function actualizarProgresoPlan(){
            ...
            const evalCap=data.sel.tipo==='presencial'&&data.sel.salaId?evaluarCapacidadSala(asig.id,data.sel.seccionId,data.sel.salaId):null;
            const capacidad=evalCap?.aplica
                ...
                : '';
            el.innerHTML=`Presencial: <span style="color:${colorP};font-weight:600;">${planesP.length}/${reqP}</span> · Virtual: <span style="color:${colorV};font-weight:600;">${planesV.length}/${reqV}</span> · Total: ${totalPlan}/${totalReq}${nota}${capacidad}`;
            el.style.display='block';
            actualizarMensajesPlanificador();
        }
        ...
        function autoAsignarBloques(asigId, secId, docId, salaId, esPresencial, offsetDia, opciones={}) {
            ...
            function evaluarCandidatoAuto(dia,bloque){
                ...
                const salaFinal=salaElegida||salaReal||ctx.SALA_TRO2_ID;
        ...
        function init(){
            ...
            const validarDestinoGrupoMovimiento=(grupo,destino)=>{
                ...
                for(const t of targets){
                    ...
                    if(seccionOcupadaVisible(t.plan.seccionId,t.dia,t.bloque,{ignorarIds:idsIgnorar})) return {ok:false,msg:'La sección ya tiene un bloque en ese horario'};
            ...
            document.addEventListener('mousemove',moverFantasmaArrastre);
            ...
            ['planArea','planCarrera','planNivel','planJornada','planSeccion','planAsignatura','planDocente','planSala','planTipo'].forEach(id=>document.getElementById(id).addEventListener('change',function(){
                ...
                if(['planArea','planCarrera','planNivel','planJornada','planSeccion','planAsignatura','planDocente','planSala','planTipo'].includes(id)){
                    if(id!=='planSala') actualizarSelectoresPlan();
                    actualizarProgresoPlan();
