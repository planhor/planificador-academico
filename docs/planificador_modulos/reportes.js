(function(){
    function createReportes(ctx){
                ...
                if(Number(grupo.alumnosTotales)>0){
                    const salasInsuficientes=planesMadre
                        ...
                        .filter(p=>{
                            ...
                            return sala && !sala.ilimitada && Number(sala.capacidad)<Number(grupo.alumnosTotales);
                        });
                    if(salasInsuficientes.length){
                        add('advertencia','Capacidad de sala',elemento,`La sala planificada podría no alcanzar para ${grupo.alumnosTotales} alumno(s) del grupo compartido.`,{seccionId:grupo.seccionMadreId,asignaturaId:grupo.asignaturaId,accionTipo:'seccion',accionId:grupo.seccionMadreId});
                    }
                }
            });
        ...
        function actualizarReporte(){
            ...
            if(resumen){
                const q=(document.getElementById('reporteBusqueda')?.value||'').trim();
                resumen.textContent = datos.length ? (q ? `${filtrados.length} de ${datos.length} registros` : `${datos.length} registros`) : '';
            }
            container.innerHTML = filtrados.length ? generarTablaReporte(filtrados, columnas) : '<p>No hay datos para este reporte.</p>';
        }
        ...
        async function exportarReporte(){
            ...
            const etiquetas={tro2:'TRO2',docenteNN:'DocenteNN',criticas:'Criticas',transversales:'Transversales',validacionPrevia:'ValidacionPrevia',gruposDictacion:'GruposDictacion',calidadHorario:'CalidadHorario',incompletas:'Incompletas',cargaDocente:'CargaDocente',excedidos:'Excedidos',comparativaHomologo:'ComparativaHomologo',conflictos:'Conflictos'};
            ...
            if(modo==='html'){
                ctx.descargarTablaExcel(nombre+'.xls', [{nombre:'Reporte', matriz:[columnas,...limpiarDatosExportacion(datosExportar)]}]);
                ctx.toast('Reporte exportado','success');
                return;
            }
            const wb=XLSX.utils.book_new();
            const datosLimpios=limpiarDatosExportacion(datosExportar);
            const ws=formatearHojaReporte(XLSX.utils.aoa_to_sheet([columnas,...datosLimpios]), columnas, datosLimpios);
        ...
        function detectarConflictos(){
            ...
            const conflictos=detectarConflictosBase();
            if(!conflictos.length){ cont.innerHTML='<p style="color:var(--success);font-size:0.85rem;">✅ Sin conflictos.</p>'; return; }
            cont.innerHTML=`
        ...
        function renderHistorial(){
            ...
            if(!eventos.length){
                cont.innerHTML=controles+'<p style="color:var(--text-secondary);font-size:0.85rem;">No hay acciones que coincidan con los filtros.</p>';
                enlazarHistorial();
                return;
            }
            cont.innerHTML=`
                ${controles}
                <div class="report-summary">${grupos.length} grupo(s), ${eventos.length} evento(s) de ${eventosBase.length} visible(s)</div>
            ...
            function enlazarHistorial(){
                ...
                if(!el) return;
                el.onchange=renderHistorial;
                    if(id==='historialBusqueda') el.onkeydown=(e)=>{ if(e.key==='Enter') renderHistorial(); };
                });
            }
        }

        function init(){
            document.getElementById('reporteTipo')?.addEventListener('change',actualizarReporte);
