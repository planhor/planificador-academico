(function(){
    function createExportaciones(ctx){
        ...
        async function resolverModoExportacion(){
            ...
            if (ctx.asegurarXLSX && await ctx.asegurarXLSX() && getXLSX()) return 'xlsx';
        ...
        function nombreComponente(plan,data){
            const rel=(data.asignaturaSeccion||[]).find(r=>r.asignaturaId===plan?.asignaturaId&&r.seccionId===plan?.seccionId);
            const comp=rel?.usaSubsecciones&&Array.isArray(rel.componentesSubseccion)
                ? rel.componentesSubseccion.find(c=>String(c.id)===String(plan?.componenteId||''))
                : null;
        ...
        function generarMatriz(planes){
            ...
            ctx.BLOQUES.forEach(b=>{
                ...
                ctx.DIAS.forEach((d,di)=>{
                    ...
                    fila.push(p?[asig?.codigo||'', nombreComponente(p,data), sala?.nombre||'', docenteEtiqueta(doc)].filter(Boolean).join(' | '):'');
                });
                mat.push(fila);
            });
            return mat;
