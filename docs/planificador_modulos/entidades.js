(function(){
    function createEntidades(ctx){
                            ...
                            return `<div style="margin-left:20px;">
                                ...
                                <div class="nested" id="secciones_${ctx.escapeAttr(n.id)}">
                                    ${grupos.map(g=>`<div class="jornada-group">
                                        <div class="jornada-header"><span>${ctx.escapeHTML(g.nombre)}</span><button class="btn btn-xs btn-nueva-seccion" data-nivel="${ctx.escapeAttr(n.id)}" data-jornada="${ctx.escapeAttr(g.jornada)}">+ Sección</button></div>
        ...
        function renderAccionesDictacion(asignatura,seccion){
            ...
            const grupo=estado.grupo;
            if(estado.estado==='dictada-aqui'){
                return `<button class="btn btn-xs btn-compartir-dictacion" data-asig="${ctx.escapeAttr(asignatura.id)}" data-grupo="${ctx.escapeAttr(grupo.id)}">Compartir</button>
        ...
        function abrirGestionDictacionSeccion(seccionId){
            const data=getData();
            const seccion=data.secciones.find(s=>s.id===seccionId);
            ...
            modal.querySelectorAll('.btn-dictar-aqui').forEach(btn=>btn.onclick=()=>{
            ...
            modal.querySelectorAll('.btn-no-aplica').forEach(btn=>btn.onclick=()=>{
                if(!confirm('¿Marcar esta asignatura como No aplica para esta sección? No se eliminarán bloques ya planificados.')) return;
        ...
        function abrirVincularDictacion(seccionId,asignaturaId){
            ...
            document.getElementById('btnConfirmarVinculo').onclick=()=>{
        ...
        function abrirCompartirDictacion(seccionMadreId,grupoId,asignaturaId){
            ...
            document.getElementById('btnConfirmarCompartir').onclick=()=>{
        ...
        function exportarWorkbookCatalogo(nombreArchivo, hojas){
            if(!window.XLSX?.utils?.book_new||!window.XLSX?.writeFile){
                ctx.toast('No se pudo cargar la librería de Excel. Revisa la conexión e intenta nuevamente.','error');
                return;
            }
        ...
        function crearPlantillaImportacionSecciones(){
            const data=getData();
            const filas=[
        ...
        function leerArchivoSecciones(file){
            if(!file) return;
        ...
        function crearPlantillaImportacionAsignaturas(){
            const data=getData();
            const headers=['Código','Nombre','Horas totales','Horas virtuales','Área','Modalidad','Condición','Distribución','Control horario','Preferencia horaria','Salas preferidas','Carrera','Nivel'];
            const filas=[
                headers,
                ['EEA401','Redes Eléctricas I',72,0,'especialidad','practica','normal','compacta','propio','flexible','315','IEL-IEL-2','N1']
            ];
            ...
            if(window.XLSX?.utils?.book_new&&window.XLSX?.writeFile){
                ...
                add('Asignaturas',filas,[14,36,14,14,16,18,28,18,20,22,28,24,12]);
        ...
        function exportarArchivoAsignaturas(){
            ...
            data.asignaturas.forEach(a=>{
                ...
                if(!relaciones.length){
                    filas.push([a.codigo,a.nombre,a.horasTotales||0,a.horasVirtuales||0,a.area||'especialidad',a.modalidad||'lectiva',a.condicion||'normal',a.distribucion||'compacta',a.controlHorario||'propio',a.preferenciaHoraria||'flexible',salas,'','']);
                    return;
                }
                relaciones.forEach(rel=>{
                    const carrera=data.carreras.find(c=>c.id===rel.carreraId);
                    const nivel=data.niveles.find(n=>n.id===rel.nivelId);
                    filas.push([a.codigo,a.nombre,a.horasTotales||0,a.horasVirtuales||0,a.area||'especialidad',a.modalidad||'lectiva',a.condicion||'normal',a.distribucion||'compacta',a.controlHorario||'propio',a.preferenciaHoraria||'flexible',salas,carrera?.codigo||'',nivel?.nombre||'']);
                });
            });
            const carrerasNiveles=[['Carrera','Código carrera','Nivel','Cómo usarlo']];
            ...
            exportarWorkbookCatalogo('Asignaturas_exportadas.xlsx',[
                {nombre:'Asignaturas',matriz:filas,cols:[14,36,14,14,16,18,28,18,20,22,28,24,12]},
        ...
        function leerArchivoAsignaturas(file){
            if(!file) return;
            if(!window.XLSX?.read || !window.XLSX?.utils?.sheet_to_json){
                ctx.toast('No se pudo cargar la librería de Excel. Revisa la conexión e intenta nuevamente.','error');
                return;
        ...
        function guardarAsignatura(id){
            ...
            const asig={...(existente||{}),id:id||ctx.genId(),codigo:cod,nombre:nom,horasTotales:ht,horasVirtuales:hv,horasPresenciales:ht-hv,bloquesPresenciales:(ht-hv)/18,bloquesVirtuales:hv/18,salasPreferidas:asignaturaTemp.salas.slice(),color:existente?.color||cols[Math.floor(Math.random()*cols.length)],
        ...
        function renderChipAsignatura(a, metaExtra=''){
            ...
            return `<span class="item-chip subject-chip" style="background:${ctx.colorSeguro(a.color,'#fff')}">
                ...
                <small class="subject-criteria-chip">${ctx.escapeHTML(etiquetas)}${metaExtra?` · ${ctx.escapeHTML(metaExtra)}`:''}${ctx.escapeHTML(alerta)}</small>
        ...
        function crearPlantillaImportacionDocentes(){
            const data=getData();
        ...
        function leerArchivoDocentes(file){
            if(!file) return;
            if(!window.XLSX?.read || !window.XLSX?.utils?.sheet_to_json){
                ctx.toast('No se pudo cargar la librería de Excel. Revisa la conexión e intenta nuevamente.','error');
                return;
        ...
        function crearPlantillaImportacionSalas(){
            const filas=[
        ...
        function leerArchivoSalas(file){
            if(!file) return;
            if(!window.XLSX?.read || !window.XLSX?.utils?.sheet_to_json){
                ctx.toast('No se pudo cargar la librería de Excel. Revisa la conexión e intenta nuevamente.','error');
                return;
        ...
        function renderSalas(){
            const data = getData();
            const contenido=data.salas.map(s=>{
                ...
                return `<span class="item-chip" style="${s.id===ctx.SALA_VIRTUAL_ID?'background:#f3e5f5;':''}${s.id===ctx.SALA_TRO2_ID?'background:#fff3e0;':''}">
                    ${ctx.escapeHTML(s.nombre)}${s.tipoSala?` <small style="font-size:0.65rem;color:var(--text-secondary)">[${ctx.escapeHTML(s.tipoSala)}]</small>`:''}${alerta} (Cap:${Number(s.capacidad)||0})
