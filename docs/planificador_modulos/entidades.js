(function(){
    function createEntidades(ctx){
        ...
        function crearGrupoPropioPorDesvinculacion({seccionId,asignaturaId,gruposOrigen,grupoOrigenPreferente=null}){
            ...
            const teniaUnSoloVinculo=unicoOrigen && (unicoOrigen.seccionesVinculadasIds||[]).length===1;
            const alumnosPropios=teniaUnSoloVinculo
                ? (Number(unicoOrigen.alumnosVinculados)||Number(unicoOrigen.alumnosVinculadosGestor)||0)
                : 0;
            gruposOrigen.forEach(g=>{
                ...
                if(antes===1 && !g.alumnosManualActivo){
                    g.alumnosVinculados=0;
                    g.alumnosTotales=Number(g.alumnosBase)||0;
        ...
        function desvincularFusionAsignatura(seccionId,asignaturaId,opciones={}){
            const data=getData();
            const seccion=data.secciones.find(s=>s.id===seccionId);
            const asignatura=data.asignaturas.find(a=>a.id===asignaturaId);
            ...
            if(!opciones.omitirConfirmacion && !confirm('Esta asignatura dejará de heredar horario desde la sección madre y se planificará aparte. No se copiarán bloques de la madre. ¿Continuar?')) return;
        ...
        function renderHistorialFusionDictacion(asignatura,seccion){
            const estado=ctx.getEstadoDictacionAsignatura?.(asignatura.id,seccion.id)||{estado:'sin-grupo'};
            const grupo=estado.grupo;
            if(!grupo?.fusionDesvinculada) return '';
            const ultimo=Array.isArray(grupo.fusionHistorial)&&grupo.fusionHistorial.length
                ? grupo.fusionHistorial[grupo.fusionHistorial.length-1]
                : null;
            const partes=[
                'Fusión desvinculada',
                ultimo?.idGestor?`ID original: ${ultimo.idGestor}`:(grupo.idGestorSeccion?`ID: ${grupo.idGestorSeccion}`:''),
                ultimo?.seccionMadreId?`Madre anterior: ${nombreSeccion(ultimo.seccionMadreId)}`:'',
                ultimo?.fecha?`Fecha: ${new Date(ultimo.fecha).toLocaleString()}`:''
            ].filter(Boolean);
        ...
        function renderEstadoDictacion(asignatura,seccion){
            ...
            if(estado.estado==='dictada-aqui'){
                ...
                return `<span class="dictation-status own">Se dicta aquí</span>
                    ${grupo.fusionDesvinculada?`<span class="dictation-status detached">Fusión desvinculada</span>`:''}
        ...
        function desvincularFusionDesdeMadre(seccionMadreId,grupoId,seccionHijaId,asignaturaId){
            ...
            if(!confirm(`La sección ${nombreSeccion(seccionHijaId)} dejará de heredar ${nombreAsignatura(asignaturaId)} desde ${nombreSeccion(seccionMadreId)}. Se conservará un registro del ID para revisión posterior. ¿Continuar?`)) return;
